...
def menu():
	...
	while choice != 5:
		...
		if choice == 1:
			...
			filePath = raw_input(
				"Provide a file path to load training and test datai.e. /home/user/data\n Assumes names as training_data.csv and test_data.csv\n")
			load_file(filePath)
	...
	save_trn = int(raw_input("Save results of training (accuracy and cost)?\n 1. Yes\n 2. No"))
	...
	save_net = int(raw_input("Save trained network after training?\n 1. Yes\n 2. No"))

	if save_net == 1:
		net_file_path = raw_input(
			"Provide a file path and name to save to i.e. /home/user/data/_networks/my_network.py")
...
# Exit (5)
python.quit()  ## load_file() function ##
...
##
def train_network(net_shape=[96, 30, 11], net_cost="quadratic", save_trn=None, trn_file_path=None, trn_file_name=None,
				  save_net=None, net_file_path=None, method=1, single_hyp=[30, 10, 0.2], iter_choice=None,
				  iter_range=None, iter_hyp=[30, 10, 0.2]):
	...
	if data_loaded == True:
		...
		if method == 1:
			...
			ev_c, ev_a, tr_c, tr_a, epo = net.SGD(training_data, single_hyp[0], single_hyp[1], single_hyp[2],
		...
		# Iterative Training
			...
			if iter_choice == "N":
				...
				while k <= upper:
					ev_c, ev_a, tr_c, tr_a, epo = net.SGD(training_data, k, iter_hyp[1], iter_hyp[2],
			...
			# Iterate through batch sizes
				...
				while k <= upper:
					ev_c, ev_a, tr_c, tr_a, epo = net.SGD(training_data, iter_hyp[0], k, iter_hyp[2],
														  evaluation_data=validation_data,
														  monitor_evaluation_accuracy=True, monitor_training_cost=True)
					if save_trn == 1:
						epo_df = pd.DataFrame(data=epo)
			...
			# Iterate through learning rates
				...
				while k <= upper:
					ev_c, ev_a, tr_c, tr_a, epo = net.SGD(training_data, iter_hyp[0], iter_hyp[1], k,
														  evaluation_data=validation_data,
														  monitor_evaluation_accuracy=True, monitor_training_cost=True)
					if save_trn == 1:
						epo_df = pd.DataFrame(data=epo)
